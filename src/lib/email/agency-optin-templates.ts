...
const DASHBOARD_URL = "https://www.specialcarer.com/dashboard/agency-optin";
...
export function renderOptInApprovedEmail(args: { name: string }): EmailContent {
  ...
  const html = shell(
    ...
    <p style="margin:24px 0">
      <a href="https://www.specialcarer.com/m/jobs" style="display:inline-block;background:#0E7C7B;color:#FFFFFF;text-decoration:none;padding:12px 20px;border-radius:9999px;font-weight:700">
        Open jobs feed
      </a>
    </p>`,
  );
  const text = [
    ...
    "",
    "https://www.specialcarer.com/m/jobs",
    "",
    "— SpecialCarer",
  ].join("\n");
  return { subject, html, text };
