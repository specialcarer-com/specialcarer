      ...
      (slug) => `
        <tr>
          ...
          <td style="padding:10px 12px;border-top:1px solid #E5E9E9;text-align:right">
            <a href="https://www.specialcarer.com/dashboard/training/${slug}" style="color:${TEAL};text-decoration:none;font-weight:600">Start &rsaquo;</a>
          </td>
        </tr>`,
    )
    .join("");
...
<html><body style="font-family:'Plus Jakarta Sans',Arial,sans-serif;background:${BG};margin:0;padding:24px;color:${TEXT}">
  <div style="max-width:600px;margin:0 auto;background:#FFFFFF;border-radius:16px;padding:32px">
    ...
    <p style="margin:32px 0">
      <a href="https://www.specialcarer.com/dashboard/agency-optin" style="display:inline-block;background:${TEAL};color:#FFFFFF;text-decoration:none;padding:14px 24px;border-radius:9999px;font-weight:700">
...
function renderText(i: GracePeriodBlastInput): string {
  const lines = [
    ...
    ...i.missingCourses.map(
      (slug) =>
        `  • ${COURSE_LABEL[slug] ?? slug} — https://www.specialcarer.com/dashboard/training/${slug}`,
    ),
    "",
