...
test("consent PDF contains candidate details and declaration", async () => {
  const consent: ReferenceConsent = {
    id: "7d5d1124-1a4c-4c85-98d7-6f91f10e9312",
    carer_id: "7d5d1124-1a4c-4c85-98d7-6f91f10e9312",
    full_name: "Aisha Khan",
    date_of_birth: "1992-03-12",
    national_insurance_number: "QQ123456C",
    signature_data_url:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADElEQVR42mNk+M/wHwAF/gL+Z4rDeAAAAABJRU5ErkJggg==",
  ...
  const contents = (
    loaded.getPages()[0].node.Contents() as unknown as {
      get(index: number): unknown;
    }
  ).get(0);
  const stream = loaded.context.lookup(contents as never) as unknown as {
  ...
  assert.match(
    pdfHex,
    new RegExp(Buffer.from("Aisha Khan").toString("hex").toUpperCase())
  );
  assert.match(
    pdfHex,
    new RegExp(
      Buffer.from("Candidate Disclosure Consent").toString("hex").toUpperCase()
    )
  );
