...
function drawHeaderAndFooter(
  ...
  for (let i = 0; i < total; i++) {
    ...
    const leftTxt = `Generated by SpecialCarer on ${fmtDateTime(input.generatedAt)}`;
...
export async function renderCarePlanPdf(
  ...
  pdf.setProducer("SpecialCarer");
  pdf.setCreator("SpecialCarer Care-Plan PDF");
