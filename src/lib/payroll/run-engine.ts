...
function previewEmailHtml(name: string, run: PayrollRun, netCents: number): string {
  ...
  <p><a href="https://www.specialcarer.com/dashboard/payslips" style="background:#0E7C7B;color:white;padding:10px 16px;border-radius:8px;text-decoration:none;">View draft payslip</a></p>
...
function payslipReadyEmailHtml(name: string, netCents: number): string {
  ...
  <p><a href="https://www.specialcarer.com/dashboard/payslips" style="background:#0E7C7B;color:white;padding:10px 16px;border-radius:8px;text-decoration:none;">View payslip</a></p>
  <p style="color:#6b7280;font-size:12px;">SpecialCarer · All Care 4 U Group Ltd</p>
  </body></html>`;
}

function payslipReadyEmailText(name: string, netCents: number): string {
  return `Hi ${name},\n\nYour payslip is ready — net pay ${gbp(netCents)} will be transferred to your bank shortly.\n\nView it: https://www.specialcarer.com/dashboard/payslips\n\nSpecialCarer · All Care 4 U Group Ltd`;
}
