...
export default function RefereeForm({
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (responseMode === "declined") {
      if (declineReason.trim().length < 5) { setErrors(["Please tell us briefly why you are declining."]); setState("err"); return; }
      setState("submitting");
      const res = await fetch("/api/references/submit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token, response_mode: "declined", decline_reason: declineReason.trim() }) });
      if (res.ok) setState("ok"); else { const json = await res.json().catch(() => ({})); setErrors([prettyError(json.error)]); setState("err"); } return;
    }
    if (responseMode === "upload") {
      if (!uploadFile) { setErrors(["Choose a reference document to upload."]); setState("err"); return; }
      ...
      if (uploadFile.size > 10 * 1024 * 1024 || !allowed.includes(uploadFile.type)) { setErrors(["Upload a PDF, Word document, PNG or JPG no larger than 10MB."]); setState("err"); return; }
      setState("submitting");
      try { const setup = await fetch("/api/references/upload-url", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token, filename: uploadFile.name, mime: uploadFile.type, size: uploadFile.size }) }); const signed = await setup.json().catch(() => ({})); if (!setup.ok || !signed.signedUrl) throw new Error(signed.error ?? "Could not prepare upload"); const put = await fetch(signed.signedUrl, { method: "PUT", headers: { "Content-Type": uploadFile.type }, body: uploadFile }); if (!put.ok) throw new Error("Could not upload document"); const res = await fetch("/api/references/submit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token, response_mode: "upload", uploaded_file_path: signed.path, uploaded_file_size: uploadFile.size, uploaded_file_mime: uploadFile.type, comment: comment.trim() || undefined }) }); const json = await res.json().catch(() => ({})); if (!res.ok || !json.ok) throw new Error(json.error ?? "Could not submit document"); setState("ok"); } catch (err) { setErrors([err instanceof Error ? prettyError(err.message) : "Could not upload document"]); setState("err"); } return;
    }
  ...
  return (
    <form onSubmit={submit} className="space-y-8">
      <fieldset className="rounded-2xl border border-[#039EA0]/20 bg-[#F4EFE6] p-4">
        <legend className="px-1 text-sm font-bold text-[#0F1416]">How would you like to respond?</legend>
        <div className="mt-2 space-y-3">
          ...
          ].map(([value, label, help]) => <label key={value} className="flex cursor-pointer items-start gap-3 rounded-xl bg-white p-3 text-sm"><input type="radio" name="response-mode" value={value} checked={responseMode === value} onChange={() => setResponseMode(value as "form" | "upload" | "declined")} className="mt-1 h-4 w-4 text-[#039EA0]" /><span><span className="block font-semibold text-[#0F1416]">{label}</span><span className="text-slate-600">{help}</span></span></label>)}
        </div>
      </fieldset>
      {responseMode === "form" && <>
      ...
      {responseMode === "upload" && <section className="space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-4"><label className="block text-sm font-semibold text-slate-800">Upload reference document<input type="file" accept=".pdf,.doc,.docx,.png,.jpg,.jpeg" onChange={(e) => setUploadFile(e.target.files?.[0] ?? null)} className="mt-2 block w-full text-sm" /></label><p className="text-xs text-slate-500">PDF, Word, PNG or JPG. Maximum 10MB.</p><label className="block text-sm font-semibold text-slate-800">Anything else you’d like to add?<textarea value={comment} onChange={(e) => setComment(e.target.value)} maxLength={2000} rows={4} className={`${inputClass} mt-1`} /></label></section>}
      {responseMode === "declined" && <section className="rounded-xl border border-amber-200 bg-amber-50 p-4"><label className="block text-sm font-semibold text-slate-800">Please tell us briefly why you’re declining. <span className="font-normal text-slate-600">This won’t be shared with the candidate.</span><textarea value={declineReason} onChange={(e) => setDeclineReason(e.target.value)} required={responseMode === "declined"} minLength={5} maxLength={1000} rows={4} className={`${inputClass} mt-2`} /></label></section>}
