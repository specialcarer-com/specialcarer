...
function asOptionalText(
  ...
  if (typeof value !== "string")
    return { value: null, error: `${field} is invalid` };
...
export function createSubmitHandler(
  ...
  return async function POST(req: Request) {
    ...
    if (responseMode !== "form") {
      ...
      const { data: row, error: referenceError } = await admin
        .from("carer_references")
        .select("id, status, token_expires_at, upload_path")
        .eq("token", token)
        .maybeSingle<{
          id: string;
          status: string;
      ...
      if (responseMode === "declined") {
        ...
        if (reason.length < 5 || reason.length > 1000)
          return invalid(
            "A decline reason of at least 5 characters is required"
          );
      ...
      const filePath =
        typeof body.uploaded_file_path === "string"
          ? body.uploaded_file_path
          : "";
      const fileSize = body.uploaded_file_size;
      const fileMime =
        typeof body.uploaded_file_mime === "string"
          ? body.uploaded_file_mime
          : "";
      const allowedMime = [
        "application/pdf",
        "application/msword",
      ...
      if (
        !row.upload_path ||
        filePath !== row.upload_path ||
        !Number.isInteger(fileSize) ||
        (fileSize as number) < 1 ||
        (fileSize as number) > 10 * 1024 * 1024 ||
        !allowedMime.includes(fileMime)
      ) {
        return invalid("A valid uploaded reference document is required");
      }

      const pathSeparator = filePath.lastIndexOf("/");
      const directory =
        pathSeparator >= 0 ? filePath.slice(0, pathSeparator) : "";
      const fileName =
        pathSeparator >= 0 ? filePath.slice(pathSeparator + 1) : filePath;
      ...
      const { data: updated, error } = await admin
        ...
        .update({
          ...
          uploaded_file_size: storedSize,
          uploaded_file_mime: storedMime,
          comment:
    ...
    const positionHeld = asOptionalText(
      body.position_held,
      120,
      "Position held"
    );
    ...
    const sponsorsVisa = asOptionalText(
      body.sponsors_visa,
      200,
      "Visa sponsorship"
    );
    const valuesExample = asOptionalText(
      body.values_example,
      2000,
      "Values example"
    );
    ...
    if (!refereeCompanyAddr.value)
      return invalid("Company address is required");
