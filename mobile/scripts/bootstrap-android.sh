...
# 1. Ensure offline webDir exists (Capacitor requires it even with remote server URL)
mkdir -p mobile/web
...
# 2. Generate android/ if not present or malformed
...
# 3. Generate app icons + splash (requires mobile/resources/icon.png)
if [ -f mobile/resources/icon.png ]; then
  ...
  echo "⚠️  mobile/resources/icon.png missing — skipping asset generation (placeholder icons)"
fi

# 4. Sync Capacitor plugins + web assets into android/
echo "🔄 Running cap sync android …"
npx cap sync android

# 5. Apply permissions, deep links, network security overlay
bash mobile/scripts/apply-android-overlay.sh

echo ""
echo "✅ Android bootstrap complete."
echo ""
echo "Next steps:"
echo "  1. Ensure ANDROID_HOME is set (Android Studio ▸ SDK Manager)"
echo "  2. cd android && ./gradlew assembleDebug"
echo "  3. npx cap run android   # or open android/ in Android Studio"
echo ""
echo "Local dev against Next.js on the host machine:"
